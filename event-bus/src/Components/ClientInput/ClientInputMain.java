...
public class ClientInputMain {
		...
		while (!done) {
			...
			try {
				...
				case "6":
					...
					break;
				case "7":
					// 학생에게 과목을 등록하는 수강신청 이벤트 전송
                    // makeRegistrationInfo 함수는 학생 ID + 과목 ID 문자열 생성
					eventBus.sendEvent(new Event(EventId.RegisterCourseForStudent, makeRegistrationInfo()));
					printLogSend(EventId.RegisterCourseForStudent);
					break;
				case "0":
	...
	private static String makeRegistrationInfo() throws IOException {
		String userInput = "";
		System.out.println("\nEnter student ID and press return (Ex. 20131234)>> ");
		userInput = new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
		System.out.println("\nEnter course ID and press return (Ex. 12345)>> ");
	...
	private static void writeMenu() {
		...
		System.out.println("7. Register a Course for a Student");
